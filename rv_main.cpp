...
BLYNK_WRITE(V17) {
	...
	char devices[][5] = {"ADC","DS0","BME"};
  	int index =  param.asInt();
    ifstream myfile;
    char temp[20];
	sprintf(temp,"%s",devices[index-1]);
    for(int i =0;i<3;i++)
    ...
    myfile >> temp;
    myfile.close();
    Blynk.virtualWrite(V16, "add", rowIndex, low, temp);

    rowIndex++;

    Blynk.virtualWrite(V18,temp);

	sprintf(str,"TST_%s",devices[index-1]);
...
BLYNK_WRITE(V15) {
	...
	switch (index ) {
		...
		case 2:
    		getCPUTemperature(&v);
    		Blynk.virtualWrite(V13,  v); 
    		Blynk.setProperty(V13,  "label",  "Pi CPU Temperature")  ;
			break;
	}
}
BLYNK_WRITE(V12) {
...
BLYNK_WRITE(V19) {
	...
	for (int index =0 ;index<3;index++){
    	...
    	myfile.open (pathFile);
    	myfile >> temp;
    	Blynk.virtualWrite(V16, "add", index, low, temp);
    	myfile.close();
	}
}
BLYNK_WRITE(V20) {
